...
export default function LoginScreen() {
  ...
  function submit() {
    ...
    login({ role, name: name.trim() || fallback, classCode: code.trim() || classroom?.code || 'LEAF-204' })
  }
    ...
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.wrap}>
        <View style={s.heroCard}>
          ...
          <Text style={s.sub}>Plant identification, lesson missions, and classroom progress in one field-learning app.</Text>
        </View>

        <View style={s.card}>
          <Text style={s.label}>Choose account type</Text>
          ...
          <Text style={s.inputLabel}>Class code</Text>
          ...
          <TouchableOpacity style={s.loginBtn} onPress={submit} activeOpacity={0.86}>
            <Text style={s.loginText}>{role === 'teacher' ? 'Open teacher dashboard' : 'Join classroom'}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </SafeAreaView>
...
const s = StyleSheet.create({
  ...
  card: { backgroundColor: C.white, borderRadius: 28, padding: 18, shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 16, shadowOffset: { width: 0, height: 8 }, elevation: 3 },
  label: { color: C.bark, fontWeight: '900', marginBottom: 10 },
  roleRow: { flexDirection: 'row', gap: 10, marginBottom: 16 },
  roleBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', borderWidth: 1.5, borderColor: C.line, borderRadius: 18, paddingVertical: 13, paddingHorizontal: 12, gap: 8 },
  roleActive: { borderColor: C.leaf, backgroundColor: '#eef8f1' },
  roleDot: { width: 14, height: 14, borderRadius: 14, backgroundColor: '#d8d0c3' },
